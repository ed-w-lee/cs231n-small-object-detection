...
def chip_image(img,coords,classes,shape=(300,300),overlap=0):
    """
    ...
    Args:
        ...
        overlap: pixels of overlap in each chip
    ...
    w_num = int((width-overlap)/(wn-overlap))
    h_num = int((height-overlap)/(hn-overlap))
    ...
    for i in range(w_num):
        for j in range(h_num):
            xmin = i*(wn-overlap)
            xmax = i*(wn-overlap) + wn
            x = np.logical_or( np.logical_and((coords[:,0]<xmax),(coords[:,0]>=xmin)),
                               np.logical_and((coords[:,2]<xmax),(coords[:,2]>=xmin)))
            out = coords[x]
            ymin = j*(hn-overlap)
            ymax = j*(hn-overlap) + hn
            y = np.logical_or( np.logical_and((out[:,1]<ymax),(out[:,1]>=ymin)),
                               np.logical_and((out[:,3]<ymax),(out[:,3]>=ymin)))
            outn = out[y]
            out = np.transpose(np.vstack((np.clip(outn[:,0]-xmin,0,wn),
                                          np.clip(outn[:,1]-ymin,0,hn),
                                          np.clip(outn[:,2]-xmin,0,wn),
                                          np.clip(outn[:,3]-ymin,0,hn))))
            ...
            chip = img[ymin:ymax,xmin:xmax,:3]
            images[k]=chip
