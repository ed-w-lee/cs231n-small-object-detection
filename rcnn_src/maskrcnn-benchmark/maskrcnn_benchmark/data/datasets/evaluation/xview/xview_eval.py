...
def do_xview_evaluation(dataset, predictions, output_folder,
    ...
    result_str = ""
    all_results = []
    for area_label, area_range in zip(area_labels, area_ranges):
        result_str += "\n__{}__\n".format(area_label.upper())
    ...
    if output_folder:
        with open(os.path.join(output_folder, "result-voc.txt"), "w") as fid:
            fid.write(result_str)
    return all_results


def eval_detection_voc(pred_boxlists, gt_boxlists, iou_thresh=0.5, use_07_metric=False,
        area_range=None, splits=None):
    ...
    Args:
        ...
        area_range: None or 2-tuple of range of areas to consider, inclusive of 1st, exclusive of 2nd
        splits: additional category splits
    Returns:
...
def calc_detection_voc_prec_rec(gt_boxlists, pred_boxlists, iou_thresh=0.5, area_range=None):
   ...
   """
    ...
    for gt_boxlist, pred_boxlist in zip(gt_boxlists, pred_boxlists):
        ...
        # filter by area
        if area_range:
            pred_area = pred_boxlist.area().numpy()
            pred_mask_a = (pred_area >= area_range[0]) & (pred_area < area_range[1])
            pred_bbox = pred_bbox[pred_mask_a]
            pred_label = pred_label[pred_mask_a]
            pred_score = pred_score[pred_mask_a]

            gt_area = gt_boxlist.area().numpy()
            gt_mask_a = (gt_area >= area_range[0]) & (gt_area < area_range[1])
            gt_bbox = gt_bbox[gt_mask_a]
            gt_label = gt_label[gt_mask_a]
